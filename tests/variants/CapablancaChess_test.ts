...
import Coords from "@/variants/capablanca-chess/constants/CapablancaCoords.ts";
...
Deno.test("Board validity #2", () => {
  const boardStr = "rnabqkbcnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNABQKBCNR";
  const board = CapablancaBoard.fromString(boardStr);
  assertEquals(board.get(Coords.get(7, 0)), board.get(Coords.get(7, 9)));
  assertEquals(board.get(Coords.get(7, 1)), board.get(Coords.get(7, 8)));
  assertEquals(board.get(Coords.get(7, 2)), CapablancaPiece.Pieces.WHITE_ARCHBISHOP);
  assertEquals(board.get(Coords.get(7, 7)), CapablancaPiece.Pieces.WHITE_CHANCELLOR);
});