...
Deno.test("entering moves #1", () => {
  ...
  game.playMove(new PawnMove(Coords.get(6, 4), Coords.get(4, 4)));
  const { board } = game.currentPosition;

  assertFalse(board.has(Coords.get(6, 4)));
  assertEquals(board.get(Coords.get(4, 4)), Pieces.WHITE_PAWN);
});

Deno.test("entering moves #2", () => {
  const game = new ChessGame();
  game
    .playMove(new PawnMove(Coords.get(6, 4), Coords.get(4, 4)))
    .playMove(new PawnMove(Coords.get(1, 3), Coords.get(3, 3)))
    .playMove(new PawnMove(Coords.get(4, 4), Coords.get(3, 3)));
  const { board } = game.currentPosition;

  assert(!board.has(Coords.get(4, 4)));
  assertEquals(board.get(Coords.get(3, 3)), Pieces.WHITE_PAWN);
});
...
Deno.test("PGN variations #1", () => {
  ...
  assertEquals(game.currentPosition.next.size, 1);
  assertEquals(game.currentPosition.next.get([...game.currentPosition.next.keys()][0])?.next?.size, 2);
});
...
Deno.test("PGN variations #2", () => {
  ...
  assertEquals([...prev.next.keys()][0].getAlgebraicNotation(prev.board, prev.legalMoves), "Bg2");
});
...
Deno.test("Real PGN #1", () => {
  ...
  assert(game.toString().includes(moveList), game.toString());
});
