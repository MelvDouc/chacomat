...
test("Parse variations.", () => {
  ...
  const game = ChessGame.fromPGN(`[Result "1-0"] ${moveStr} 1-0`);
  const pgn = game.toPGN();
  expect(pgn).toInclude(moveStr);
});

test("Parse long game back and forth.", async () => {
  const input = await Bun.file(`${process.cwd()}/pgn-files/long-engine-game1.pgn`).text();
  const game = ChessGame.fromPGN(input);
  const pgn = game.toPGN();

  for (let i = 0; i < pgn.length; i++) {
    if (pgn[i] !== input[i]) {
      console.log(`<< ${pgn.slice(Math.max(i - 10, 0), i + 10)} >>`);
...
test("Parse annotations.", () => {
  ...
  expect(game.toPGN()).toInclude("1.d4 e6 2.c4 Bb4+ $5 3.Qd2 $4 Bxd2+ $21");
});

test("Parse null moves.", () => {
  ...
  game
    ...
    .playNullMove();
  game.playNullMove();
  expect(game.toPGN()).toInclude("1.-- e5 2.e4 --");
});