...
Deno.test("string to board", () => {
  ...
  const blackPieceRank = Array.from({ length: 8 }, (_, y) => board.get(board.Coords.get(0, y)));
  const whitePieceRank = Array.from({ length: 8 }, (_, y) => board.get(board.Coords.get(7, y)));
...
Deno.test("board to string", () => {
  ...
  for (let y = 0; y < 8; y++) {
    board.set(board.Coords.get(1, y), ShatranjPiece.Pieces.BLACK_PAWN);
    board.set(board.Coords.get(6, y), ShatranjPiece.Pieces.WHITE_PAWN);
  }
