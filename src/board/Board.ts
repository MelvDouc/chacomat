...
export default class Board {
  ...
  getAttackedCoordsSet(color: ChacoMat.Color) {
    ...
    for (const [srcCoords, piece] of this.#pieces)
      if (piece.color === color)
        for (const destCoords of piece.getAttacks(this, srcCoords))
          set.add(destCoords);

    return set;
  }

  isKingEnPrise(color: ChacoMat.Color) {
    const kingCoords = this.getKingCoords(color);

    for (const [srcCoords, piece] of this.#pieces)
      if (piece.color !== color && piece.getAttacks(this, srcCoords).includes(kingCoords))
        return true;
   ...
   */
  mirror({ horizontal, vertical, colors }: BoardMirrorParams) {
    const clone = new Board();
