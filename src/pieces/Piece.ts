...
export default class Piece {
  ...
  getAttackedCoords(board: ChacoMat.Board, srcCoords: ChacoMat.Coords) {
    if (this.isShortRange())
      return shortRangePeers[srcCoords.x][srcCoords.y][this.value];

    const attackedCoords: ChacoMat.Coords[] = [];

    for (const peers of longRangePeers[srcCoords.x][srcCoords.y][this.value]) {
